...
(function () {
  ...
  const EN = {
    ...
    "proj.kraftonLogChosen": "I split the video into chunks, processed them in parallel, retried only failed chunks and saved intermediate results to files.",
    "proj.kraftonLogRationale": "Smaller work units let the pipeline continue after a partial failure without rerunning the entire video.",
    ...
    "collab.checkmateDesc": "A system that reviews unfair contract clauses with RAG on Azure OpenAI and AI Search. The flow uses a Rule Engine for clear violations and sends only ambiguous clauses to the LLM, constraining calls and source handling.",
    ...
    "collab.checkmateLogChosen": "Azure Document Intelligence extracted text and coordinates. The Rule Engine handled explicit violations, then RAG and the LLM analyzed clauses that needed interpretation.",
  ...
  function initProjectModal() {
    ...
    const traceLabels = [
      ["ROLE", "PROBLEM", "BUILD", "DECISION", "TEST", "OUTPUT"],
      ["ROLE", "DATA", "DECISION", "EVALUATE", "OUTPUT"],
      ["ROLE", "GATE", "DECISION", "OUTPUT"],
      ["CONSTRAINT", "BUILD", "DECISION", "OUTPUT"],
      ["ROLE", "UX", "RESULT", "DECISION", "OUTPUT"],
      ["ROLE", "PIPELINE", "DECISION", "RESULT", "OUTPUT"],
      ["ROLE", "PIPELINE", "DECISION", "BOUNDARY", "OUTPUT"]
    ];
    ...
    const openCard = (card) => {
      ...
      if (sourceImage) {
        ...
        visual.append(image, createElement("span", "project-console-scan"));
      } else {
        visual.classList.add("no-image");
        visual.append(createElement("span", "project-console-scan"));
      }
